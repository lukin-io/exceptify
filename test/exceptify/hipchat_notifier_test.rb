...
module HipChat
  class Client
    def initialize(_api_token, _options)
    end

    def [](room_name)
      Room.new if room_name == "room_name"
    end
  end

  class Room
    def send(_from, _message, _options)
    end
  end
end
...
class HipchatNotifierTest < ActiveSupport::TestCase
  test "should send hipchat notification if properly configured" do
    ...
    hipchat = Exceptify::HipchatNotifier.new(options)
  ...
  test "should not send hipchat notification if badly configured" do
    ...
    hipchat = Exceptify::HipchatNotifier.new(wrong_params)
